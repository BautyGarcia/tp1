...
void mostrarEmpleados(const Departamento& dept) {
    cout << "Empleados de " << dept.nombre << ": " << endl;
    for (const Empleado& e : dept.getEmpleados()) {
        cout << e.nombre << endl;
    }
}

void mostrarGerentes(const CentralRegional& central) {
    cout << "Gerentes de " << central.nombre << ": " << endl;
    cout << "Gerentes altos: " << endl;
    for (const GerenteAlto& g : central.getGerentesAlto()) {
        cout << g.nombre << endl;
    }
    cout << "Gerentes medios: " << endl;
    for (const GerenteMedio& g : central.getGerentesMedio()) {
        cout << g.nombre << endl;
    }
}
...
int main() {
    ...
    CentralRegional VolkswagenLatam("Volkswagen Latam");
    ...
    Empresa VolkswagenArgentina("Volkswagen Argentina", "Av. Corrientes 123");
    Empresa VolkswagenUruguay("Volkswagen Uruguay", "Av. Corrientes Bo 123");
    
    // Creo los departamentos de las empresas
    Departamento deptITArgentina("IT", "Av. Corrientes 123");
    Departamento deptRHArgentina("RH", "Av. Corrientes 123");
    Departamento deptITUruguay("IT", "Av. Corrientes Bo 123");
    Departamento deptRHUruguay("RH", "Av. Corrientes Bo 123");

    // Creo gerentes
    GerenteAlto gerenteAltoArgentina("Juan", "Corrector de TPs", 1, 100000, 15, ManagerLevel::ALTO);
    GerenteAlto gerenteAltoUruguay("Maria", "Lavaplatos", 4, 100000, 15, ManagerLevel::ALTO);
    GerenteMedio gerenteMedioArgentina("Pedro", "Marinero", 1, 100000, 15, ManagerLevel::MEDIO);
    GerenteMedio gerenteMedioUruguay("Ana", "Chofer", 2, 100000, 15, ManagerLevel::MEDIO);

    // Creo los empleados
    Empleado empITArgentina("Juan", "Corrector de TPs", 1, 100000);
    Empleado empRHArgentina("Maria", "Lavaplatos", 4, 100000);
    Empleado empITUruguay("Pedro", "Marinero", 1, 100000);
    Empleado empRHUruguay("Ana", "Chofer", 2, 100000);
    ...
    cout << "Cantidad de empleados en la central regional: " << VolkswagenLatam.getCantEmpleados() << endl;
