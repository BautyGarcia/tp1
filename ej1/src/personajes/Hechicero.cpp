...
Hechicero::Hechicero(string nombre, int mana, int vida, int armadura, int magicResistance, pair<unique_ptr<IArma>, unique_ptr<IArma>> armas) : 
    Mago(nombre, mana, vida, armadura, magicResistance, std::move(armas)) 
{}

void Hechicero::mostrarInfo() const {
    cout << "Hechicero: " << this->getNombre() << endl;
